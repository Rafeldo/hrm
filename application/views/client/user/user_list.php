...
<div class="nav-tabs-custom">
    ...
    <div class="tab-content bg-white">
            ...
            <table class="table table-striped DataTables " id="DataTables" cellspacing="0" width="100%">
                ...
                <tbody>                    
                    ...
                    if (!empty($all_user_info)): foreach ($all_user_info as $v_user) :
                            ...
                            <tr>
                                <td><img style="width: 36px;margin-right: 10px;" src="<?= base_url() ?><?= $v_user->avatar ?>" class="img-circle"></td>
                                <td><?php echo $v_user->fullname ?></td>
                                <td><?= $v_user->username ?></td>                                
                                <td><?= $v_user->phone ?></td>                                
                                <td><?= $v_user->mobile ?></td>                                
                                <td><?= $v_user->skype ?></td>                                
                                <?php
                                if ($v_user->last_login == '0000-00-00 00:00:00') {
                                    $login_time = "-";
                                } else {
                                    $login_time = strftime(config_item('date_format') . " %H:%M:%S", strtotime($v_user->last_login));
                                }
                                ?>
                                <td><?= $login_time ?> </td>
                                <td>
        ...
        <div class="tab-pane <?= $active == 2 ? 'active' : ''; ?>" id="new">
            <form role="form" id="userform" enctype="multipart/form-data" action="<?php echo base_url(); ?>client/user/save_user" method="post" class="form-horizontal form-groups-bordered">
                <div class="row">
                    <div class="col-sm-6">
                        <?php
